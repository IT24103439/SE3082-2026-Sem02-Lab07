...
int main(int argc, char **argv) {
    ...
    /* Step 2: Only root allocates the full array */
    if (rank == 0) {
    ...
    /* Step 2: All processes allocate a local chunk of size N / size */
    int *local_chunk = (int *)malloc(chunk_size * sizeof(int));

    double start = MPI_Wtime();

    /* Step 3: Replace MPI_Bcast with MPI_Scatter */
    /* sendcount is chunk_size (elements per rank), not N! */
    MPI_Scatter(array, chunk_size, MPI_INT,
                local_chunk, chunk_size, MPI_INT,
                0, MPI_COMM_WORLD);

    /* Step 4: Sum over local_chunk[0 .. chunk_size) */
    ...
    /* Step 5: Keep Send/Recv unchanged */
    if (rank != 0) {
