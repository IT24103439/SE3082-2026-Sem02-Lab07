...
int main(int argc, char **argv) {
    ...
    long long formula_expected = K * (K + 1) / 2;

    printf("Rank %d: local_sum = %lld, prefix_sum = %lld, sum_before_me = %lld | Valid? %s\n",
           rank, local_sum, prefix_sum, sum_before_me,
           prefix_sum == formula_expected ? "YES" : "NO");

    if (rank == size - 1) {
        double elapsed = MPI_Wtime() - start;
        long long expected_total = (long long)N * (N + 1) / 2;
        printf("\n[Scan] Final sum (from rank %d) = %lld\n", rank, prefix_sum);
